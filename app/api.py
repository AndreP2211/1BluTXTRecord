...
class Api:
    ...
    def update_address(self, subdomain :str, rrtype: str, new_target:str) -> bool:
        """Updates one DNS record on 1blu."""
        ...
        logging.debug(f"Updating record: '{hostname}' [{rrtype}] to new value: '{new_target}'.")
        num_updated = 0
        ...
        if num_updated == 0:
            logging.error(f"No matching {rrtype} record found for '{hostname}'.")
            return False

        if(not self.push_records()):
            logging.error("Updating DNS record failed: Pushing records failed.")
            return False
