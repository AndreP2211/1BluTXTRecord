...
This container renews Let's Encrypt certificates with the DNS-01 challenge. It writes Certbot's current challenge value to the existing `_acme-challenge` TXT record at 1Blu. It does not update A or AAAA records.
...
The `_acme-challenge` TXT record must already exist in the 1Blu DNS editor. The container updates its `target` value.

Create the first certificate once, for example with Certbot's manual DNS challenge and the same authentication hook:

```
docker compose run --rm 1blu-ddns certbot certonly --manual --preferred-challenges dns \
   --manual-public-ip-logging-ok --manual-auth-hook /app/acme-auth-hook.sh \
   -d example.de
```

After that, start the service periodically to renew the certificate:

```
docker compose run --rm 1blu-ddns
```
...
Certbot calls `/app/acme-auth-hook.sh` during the DNS-01 challenge. The hook receives the current value in `CERTBOT_VALIDATION`, logs into 1Blu, and updates the TXT record `_acme-challenge` for the configured domain. Certbot then waits for DNS propagation and validates the challenge.
